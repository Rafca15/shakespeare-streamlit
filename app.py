...
def tokenize(text):
    # Example: Convert text to a sequence of token IDs
    return torch.tensor([ord(c) for c in text], dtype=torch.long).unsqueeze(0)

def detokenize(tokens):
    # Example: Convert token IDs back to text
    return ''.join([chr(token) for token in tokens.squeeze().tolist()])

# Streamlit app
def main():
    st.title("Bigram Language Model Chatbot")
